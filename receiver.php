...
require_once IA_ROOT . '/addons/yihe_jifenbao/weixinutil.class.php';
class Yihe_jifenbaoModuleReceiver extends WeModuleReceiver
{
    public function receive()
    {
        ...
        if ($this->message['event'] == 'subscribe') {
            $status1 = 1;
            pdo_update('yihe_jifenbao_share', array('follow' => $status1), array('openid' => $mc['uid'], 'weid' => $mc['uniacid']));
            ...
            $zfs = $cfg['yihe_jifenbao_fansnum'] + $fanssum;
            $url = $_W['siteroot'] . str_replace('./', 'app/', $this->createMobileurl('kending', array('uid' => $mc['uid'])));
            $msg = htmlspecialchars_decode($cfg['yihe_jifenbao_usr']);
        ...
        if ($this->message['msgtype'] == 'event') {
            if ($this->message['event'] == 'unsubscribe') {
                $reply = pdo_fetch('select * from ' . tablename('yihe_jifenbao_poster') . ' where weid=:weid order by id asc limit 1', array(':weid' => $mc['uniacid']));
                if ($reply['rscore'] == 0) {
                    die;
                }
                ...
                $m_fans = pdo_fetch('select * from ' . tablename('yihe_jifenbao_share') . ' where weid=:weid AND from_user=:from_user order by id asc limit 1', array(':weid' => $mc['uniacid'], ':from_user' => $this->message['from']));
                if ($m_fans['helpid'] && empty($m_fans['hasdel'])) {
                    $s_fans = pdo_fetch('select * from ' . tablename('yihe_jifenbao_share') . ' where weid=:weid AND openid=:openid order by id asc limit 1', array(':weid' => $mc['uniacid'], ':openid' => $m_fans['helpid']));
                    if ($s_fans['follow'] == 0) {
                        die;
                    }
                    if ($cscore3) {
                        mc_credit_update($s_fans['openid'], 'credit1', -$cscore3, array($s_fans['openid'], '2级取消关注'));
                    }
                    if ($cscorehb3) {
                        mc_credit_update($s_fans['openid'], 'credit2', -$cscorehb3, array($s_fans['openid'], '2级取消关注'));
                    }
                    ...
                    if ($s_fans['helpid']) {
                        $p_fans = pdo_fetch('select * from ' . tablename('yihe_jifenbao_share') . ' where weid=:weid AND openid=:openid order by id asc limit 1', array(':weid' => $mc['uniacid'], ':openid' => $s_fans['helpid']));
                        if ($s_fans['follow'] == 0) {
                            die;
                        }
                        if ($pscore3) {
                            mc_credit_update($s_fans['openid'], 'credit1', -$pscore3, array($s_fans['openid'], '3级取消关注'));
                        }
                        if ($pscorehb3) {
                            mc_credit_update($s_fans['openid'], 'credit2', -$pscorehb3, array($s_fans['openid'], '3级取消关注'));
                        }
                ...
                $status = 0;
                pdo_update('yihe_jifenbao_share', array('follow' => $status, 'hasdel' => 1), array('openid' => $mc['uid'], 'weid' => $mc['uniacid']));
            }
        }
    }
    public function formot_content($content = '', $fansusr)
    {
        global $_W;
        if (empty($content)) {
            return $content;
        ...
        load()->model('mc');
        $fansusr = mc_fetch($this->message['from']);
        $replace = array('a' => array('replace' => '#昵称#', 'name' => 'nickname'), 'b' => array('replace' => '#积分#', 'name' => 'credit1'), 'c' => array('replace' => '#余额#', 'name' => 'credit2'), 'd' => array('replace' => '#国家#', 'name' => 'nationality'), 'e' => array('replace' => '#省#', 'name' => 'resideprovince'), 'f' => array('replace' => '#市#', 'name' => 'residecity'));
