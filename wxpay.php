<?php
  if(!empty($cfg['mchid'])){
	...
	$redpk->setParameter("total_amount",$kjfabc1);//发红包，每个红包金额必须大于1元，小于200元
	...
	try {
		...
		if($redpkresult["return_code"]=="SUCCESS"){
			...
			if($redpkresult["result_code"]=="FAIL"){
                ...
                mc_credit_update($fans['uid'],'credit1',-$kjfabc,array($fans['uid'],'兑换:'.$goods_info['title']));//记录积分变动 会员积分操作(记录所有的积分变动) 
				if(IS_DEBUG){
		...
		pdo_insert('yihe_jifenbao_paylog',array("uniacid"=>$_W["uniacid"],"dwnick"=>$member['nickname'],"dopenid"=>$_W['fans']['from_user'],"dtime"=>time(),"dcredit"=>$kjfabc,"dtotal_amount"=>$kjfabc1,"dmch_billno"=>$mch_billno,"dissuccess"=>$dissuccess,"dresult"=>$dresult));        
